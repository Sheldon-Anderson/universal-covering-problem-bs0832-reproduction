...
![三测试集归一化摆放](assets/figures/geometry.png)
...
![证书流程](assets/figures/certificate_flow.png)
...
| 阶段 | 对应开发版本 | 作用 |
|---|---:|---|
| V106 | v0.10.6 | Branch-B 域重放和 kernel-closure 检查 |
| V107 | v0.10.7 | 独立重放和分块哈希审计 |
| V108 | v0.10.8 | 复现闭合和证明义务绑定 |
| V109 | v0.10.9 | 最终签收裁决和作者自审验证 |
...
| 路径 | 作用 |
|---|---|
| `assets/figures/` | README 中使用的说明图 |
| `app/domain/` | 证书读取、哈希检查、签收验证、分阶段重放和最终验证的核心逻辑 |
| `scripts/` | 用户运行入口，建议用 `python -m scripts.xxx` 执行 |
| `inputs/` | 分阶段复现所需的源证书压缩包 |
| `certificate/` | 最终证书、作者自审文件、manifest 和校验清单 |
| `certificate/intermediate/` | V106--V108 参考反馈包，用于比对和 reference-signed 验证 |
| `paper/` | 配套论文 PDF |
| `runs/` | 脚本运行后自动生成的输出目录，不需要手工创建或提交 |
...
`reference-signed` 模式检查仓库中绑定好的参考 V108 文件和作者自审文件。这是快速最终验证使用的路径。

`generated-chain` 模式使用重新生成的 V108-style 文件。由于 ZIP 文件的外层 SHA256 可能因时间戳、压缩参数或文件顺序不同而变化，仓库自带的作者自审文件不会自动应用到重新生成的 ZIP 文件。

## 13. 为什么重新生成的 ZIP 不一定逐字节相同？

ZIP 文件不仅保存文件内容，还可能保存压缩时间、文件顺序、压缩参数等元数据。即使两次运行得到的内部 CSV/JSON 内容等价，外层 ZIP 文件本身的 SHA256 也可能不同。

因此，本仓库不把“外层 ZIP 是否逐字节相同”作为唯一复现标准。分阶段复现时，脚本会检查更关键的内部内容：状态文件、schema 检查、replay 行数、边界审计和关键内容哈希。阶段摘要会同时记录新生成 ZIP 与参考 ZIP 的 SHA256，用来追踪来源和比对。
